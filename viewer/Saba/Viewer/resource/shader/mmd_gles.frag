...
float SampleShadowMap(sampler2D shadowMap, vec4 shadowCoord)
{
    ...
    float bias = 0.005;
    return step(projCoord.z - bias, depth);
}
...
void main()
{
	...
	if (u_ShadowMapEnabled != 0)
	{
		...
		{
			visibility = SampleShadowMap(u_ShadowMap0, vs_shadowMapCoord[0]);
		}
		else if (u_ShadowMapSplitPositions[1] <= z && z < u_ShadowMapSplitPositions[1 + 1])
		{
			visibility = SampleShadowMap(u_ShadowMap1, vs_shadowMapCoord[1]);
		}
		else if (u_ShadowMapSplitPositions[2] <= z && z < u_ShadowMapSplitPositions[2 + 1])
		{
			visibility = SampleShadowMap(u_ShadowMap2, vs_shadowMapCoord[2]);
		}
		else if (u_ShadowMapSplitPositions[3] <= z && z < u_ShadowMapSplitPositions[3 + 1])
		{
			visibility = SampleShadowMap(u_ShadowMap3, vs_shadowMapCoord[3]);
		}
		ln *= (1.0 - visibility);
	}
